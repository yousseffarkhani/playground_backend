...
// 	t.Error("Response should contain cookies")
// }
// for _, c := range resp.Cookies() {
// 	if c.Expires.Sub(time.Now()) < 0*time.Minute {
// 		t.Errorf("Cookie is expired")
// 	}
// 	if c.Value == "" {
// 		t.Fatal("Cookie should contain JWT value")
// 	}
...
/* func TestUnsetJWT(t *testing.T) {
	...
	_, err = client.Get(svr.URL + "/login")
	// resp, err := client.Get(svr.URL + "/login")
	if err != nil {
		t.Errorf("Couldn't get a response, %s", err)
	}
	// url, _ := resp.Location()
	// fmt.Println(client.Jar.Cookies(url))

	_, err = client.Get(svr.URL + "/logout")
	if err != nil {
		t.Errorf("Couldn't get a response, %s", err)
	}
} */
